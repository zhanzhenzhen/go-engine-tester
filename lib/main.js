...
let processQueueSize = config.processQueueSize;
if (processQueueSize === undefined) {
    processQueueSize = 0;
}
...
if (!(
    ...
    (typeof processQueueSize === "number" && processQueueSize >= 0 && processQueueSize <= 10)
)) {
...
let playGame = async swaps => {
    ...
    if (processQueue.length > 0) {
        queueItem = processQueue.shift();
        controller1 = queueItem.controller1;
        controller2 = queueItem.controller2;
    }
    else {
        controller1 = new Controller(config1.command, config1.args, config1.spawnOptions);
        controller2 = new Controller(config2.command, config2.args, config2.spawnOptions);
    }
...
let getBlackWinCount = () => results.filter(m => m.winnerColor === "b").length;
let getFirstBlackWinCount = () =>
    results.filter(m => m.winnerConfig.name === "first" && m.winnerColor === "b").length
;

let printFirstWinRate = () => {
    let firstWinCount = getFirstWinCount();
    let winRate = (firstWinCount / results.length).toFixed(4);
    let blackWinCount = getBlackWinCount();
    let firstBlackWinCount = getFirstBlackWinCount();
    console.log(`First wins ${firstWinCount} out of ${results.length}. Win rate: ${winRate}`);
    console.log(`First wins ${firstBlackWinCount} out of ${blackWinCount} black-wins.`);
};
...
let sendReport = () => {
    ...
    // Make `totalCount` always even.
    if (totalCount % 2 === 1) {
        if (results[totalCount - 1].winnerConfig.name === "first") {
            firstWinCount--;
        }
        totalCount--;
    }
    ...
    return got(
        `${report.uriPrefix}?testerId=${testerId}&firstWins=${firstWinCount}&total=${totalCount}`,
